...
public class HuffmanDecompressionTest {
    ...
    @Test
    public void testDecompression_NormalCase() throws IOException {
        File originalFile = tempFolder.newFile("testFile.txt");
        BufferedWriter bw = new BufferedWriter(new FileWriter(originalFile));
        bw.write("abcaba");
        bw.close();
        ...
        String returnedFile = HuffDecompress.decompression(file.getPath());

        byte[] expected = getByteArray(originalFile.getPath());
    ...
    @Test
    public void testDecompression_FileNotFound() throws IOException {
        assertThrows(RuntimeException.class, () -> HuffDecompress.decompression("FileNotFound.txt"));
    }
}